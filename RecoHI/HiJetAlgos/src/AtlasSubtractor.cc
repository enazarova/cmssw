#include "RecoHI/HiJetAlgos/interface/AtlasSubtractor.h"
...
void AtlasSubtractor::rescaleRMS(double s){
...
void AtlasSubtractor::offsetCorrectJets()
{
...
void AtlasSubtractor::subtractPedestal(vector<fastjet::PseudoJet> & coll)
{
...
void AtlasSubtractor::calculatePedestal( vector<fastjet::PseudoJet> const & coll )
{
...
double AtlasSubtractor::getEt(const reco::CandidatePtr & in) const {
...
double AtlasSubtractor::getEta(const reco::CandidatePtr & in) const {
   const CaloTower* ctc = dynamic_cast<const CaloTower*>(in.get());
   const GlobalPoint& pos=geo_->getPosition(ctc->id());
   double eta = pos.eta();
   return eta;
